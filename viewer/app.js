...
function edgeGeom(e, lw) {
  ...
  const x1 = e.a.x + ux * rad(e.a) + px, y1 = e.a.y + uy * rad(e.a) + py;
  const x2 = e.b.x - ux * (rad(e.b) + 2) + px, y2 = e.b.y - uy * (rad(e.b) + 2) + py;
...
function draw() {
  ...
  sim.edges.forEach(e => {
    ...
    ctx.lineWidth = lw;
    if (e.a === e.b) {
      ctx.beginPath(); ctx.arc(e.a.x + 14, e.a.y - 14, 13, 0, Math.PI * 2); ctx.stroke();
      return;
    }
...
function exportMapSVG() {
  ...
  svg+=`<text x="${minX+20}" y="${maxY+10}" font-size="11">円はテーマの全論文、矢印は曝露→アウトカムとして解析した論文数。因果を意味しません。データ源 ${esc(state.study||"すべて")} / データの年 ${esc(state.wave||"すべて")}</text></g></svg>`;
...
function ldraw() {
  ...
  const sel = state.listSel, ink = cssVar("--canvas-ink", "#2f3437"), rose = document.documentElement.dataset.theme === "dark" ? "#FF5A3D" : "#F21A00"  /* つながりの図は Zissou1 の赤 */;
  lsim.edges.forEach(e => {
